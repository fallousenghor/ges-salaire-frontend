          ...
          {/* Logo de l'entreprise */}
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex flex-col">
                <span className="font-semibold text-gray-700">{entreprise?.nom ?? ''}</span>
                {/* debug URL hidden */}
              </div>
            </div>
          )}
      ...
      <div className="flex items-center gap-4">
        <div className="hidden md:flex items-center bg-gray-100 rounded-full px-4 py-2">
          ...
          <input 
            ...
            className="bg-transparent border-none outline-none ml-2 text-gray-700 placeholder-gray-400 w-48"
          />
        </div>
        <div className="relative">
          <button 
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-72 bg-white text-gray-800 rounded-xl shadow-2xl p-4 z-50">
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-orange-50 rounded-lg hover:bg-orange-100 transition-all cursor-pointer">
        ...
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-orange-500 rounded-full flex items-center justify-center font-bold shadow cursor-pointer hover:scale-110 transition-transform">
            {user ? user.email.slice(0, 2).toUpperCase() : 'SA'}
          </div>
          <span className="text-gray-700 font-medium text-sm">{user ? user.email : ''}</span>
        </div>
      </div>
    </nav>
  );
